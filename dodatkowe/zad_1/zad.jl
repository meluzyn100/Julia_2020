...
function f(z, root, n)#x: liczba zespolona, root: stopień pierwiastka np. dla √ root =2, n: ilość pierwiastków jakie chcemy znaleść
    ...
    if n <= root#każda loiczba zespolona ma n-równych pierwiastków gdzie n-stopień pierwiastka
        valiu =[r*exp(im*(alpha+2*k*pi)/root) for k = 1:n]
    else
        valiu =[r*exp(im*(alpha+2*k*pi)/root) for k = 1:root]
    end
